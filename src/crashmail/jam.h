...
typedef unsigned char  JAM_uchar;    /* must be  8 bits wide */
typedef unsigned short JAM_ushort;   /* must be 16 bits wide */
typedef unsigned long  JAM_ulong;    /* must be 32 bits wide */
...
*/
typedef struct {
    JAM_uchar  Signature[4];      /* <J><A><M> followed by <NUL> */
    JAM_ulong  DateCreated;       /* Creation date */
    JAM_ulong  ModCounter;        /* Last processed counter */
    JAM_ulong  ActiveMsgs;        /* Number of active (not deleted) msgs */
    JAM_ulong  PasswordCRC;       /* CRC-32 of password to access */
    JAM_ulong  BaseMsgNum;        /* Lowest message number in index file */
    JAM_uchar  RSRVD[1000];       /* Reserved space */
} s_JamBaseHeader;
...
typedef struct {
    JAM_uchar  Signature[4];              /* <J><A><M> followed by <NUL> */
    JAM_ushort Revision;                  /* CURRENTREVLEV */
    JAM_ushort ReservedWord;              /* Reserved */
    JAM_ulong  SubfieldLen;               /* Length of Subfields */
    JAM_ulong  TimesRead;                 /* Number of times message read */
    JAM_ulong  MsgIdCRC;                  /* CRC-32 of MSGID line */
    JAM_ulong  ReplyCRC;                  /* CRC-32 of REPLY line */
    JAM_ulong  ReplyTo;                   /* This msg is a reply to.. */
    JAM_ulong  Reply1st;                  /* First reply to this msg */
    JAM_ulong  ReplyNext;                 /* Next msg in reply chain */
    JAM_ulong  DateWritten;               /* When msg was written */
    JAM_ulong  DateReceived;              /* When msg was received/read */
    JAM_ulong  DateProcessed;             /* When msg was processed by packer */
    JAM_ulong  MsgNum;                    /* Message number (1-based) */
    JAM_ulong  Attribute;                 /* Msg attribute, see "Status bits" */
    JAM_ulong  Attribute2;                /* Reserved for future use */
    JAM_ulong  TxtOffset;                 /* Offset of text in text file */
    JAM_ulong  TxtLen;                    /* Length of message text */
    JAM_ulong  PasswordCRC;               /* CRC-32 of password to access msg */
    JAM_ulong  Cost;                      /* Cost of message */
} s_JamMsgHeader;
...
*/
typedef struct {
    JAM_ushort LoID;       /* Field ID, 0 - 0xffff */
    JAM_ushort HiID;       /* Reserved for future use */
    JAM_ulong  DatLen;     /* Length of buffer that follows */
    JAM_uchar* Buffer;     /* DatLen bytes of data */
} s_JamSubfield;
...
*/
typedef struct {
    JAM_ulong  UserCRC;    /* CRC-32 of destination username */
    JAM_ulong  HdrOffset;  /* Offset of header in .JHR file */
} s_JamIndex;
...
*/
typedef struct {
    JAM_ulong  UserCRC;     /* CRC-32 of user name (lowercase) */
    JAM_ulong  UserID;      /* Unique UserID */
    JAM_ulong  LastReadMsg; /* Last read message number */
    JAM_ulong  HighReadMsg; /* Highest read message number */
} s_JamLastRead;
...
typedef struct {
    ...
    JAM_ulong LastUserPos_I;   /* last position of lastread record */
    JAM_ulong LastUserId_I;    /* userid for the last read lastread record */
} s_JamBase;
...
/* mbase.c */
int JAM_OpenMB          ( JAM_uchar*                Basename_PC,
			  s_JamBase** 		NewArea_PPS );
int JAM_CloseMB		( s_JamBase* 		Area_PS );
int JAM_CreateMB        ( JAM_uchar*                Basename_PC,
                          JAM_ulong                 BaseMsg_I,
			  s_JamBase** 		NewArea_PPS );
int JAM_RemoveMB	( s_JamBase* 		Area_PS,
                          JAM_uchar*                Basename_PC );
...
int JAM_FindUser	( s_JamBase* 		Area_PS,
                          JAM_ulong                 UserCrc_I,
                          JAM_ulong                 StartMsg_I,
                          JAM_ulong*                MsgNo_PI );
int JAM_GetMBSize	( s_JamBase* 		Area_PS,
                          JAM_ulong*                Messages_PI );
...
/* crc32.c */
JAM_ulong JAM_Crc32         ( JAM_uchar* Buffer_PC, JAM_ulong Length_I );
